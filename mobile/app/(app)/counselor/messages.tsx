...
function computeUnread(messages: Message[]) {
  ...
  const lastCounselorIdx = reversed.findIndex(m => m.sender_role === 'counselor');
  if (lastCounselorIdx === -1) return messages.filter(m => m.sender_role === 'student').length;
  return lastCounselorIdx;
}
...
export default function CounselorMessagesScreen() {
  ...
  const load = useCallback(async () => {
    ...
    const list: Student[] = (studentsData ?? []).map(p => ({
      id: p.id,
    ...
    const keys = list.map(s => conversationKey(user.id, s.id));
  ...
  useEffect(() => {
    ...
    const key = conversationKey(user.id, selectedStudentId);

    channelRef.current = supabase
      .channel(`counselor_msgs_${key}`)
      .on(
        'postgres_changes',
  ...
  async function sendMessage() {
    if (!input.trim() || !selectedStudentId || !user?.id || sending) return;
    const key = conversationKey(user.id, selectedStudentId);
    ...
    const { data, error } = await supabase
      .from('messages')
      .insert({ conversation_key: key, sender_id: user.id, sender_role: 'counselor', content })
      .select()
      .single();
    ...
    return (
      <SafeAreaView style={s.container}>
        ...
        <View style={s.threadHeader}>
          <TouchableOpacity onPress={() => setSelectedStudentId(null)} style={s.backBtn}>
            <Text style={s.backText}>← Back</Text>
          </TouchableOpacity>
          <View style={{ flex: 1 }}>
            <Text style={s.threadName}>{student.firstName} {student.lastName}</Text>
            {student.gradeLevel && <Text style={s.threadSub}>Grade {student.gradeLevel}</Text>}
          </View>
        </View>
        ...
        <KeyboardAvoidingView style={s.flex} behavior="padding">
          <FlatList
            ...
            renderItem={({ item }) => {
              const isMe = item.sender_role === 'counselor';
              return (
  ...
  return (
    <SafeAreaView style={s.container}>
      ...
      {students.length === 0 ? (
        <View style={s.center}>
          <Text style={s.muted}>No approved students at your school yet.</Text>
        </View>
      ) : (
        <FlatList
          data={students}
            ...
            return (
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  ...
                  {item.gradeLevel && (
                    <Text style={[s.muted, { fontSize: 12 }]}>Grade {item.gradeLevel}</Text>
                  )}
                  {lastMsg && (
                    <Text style={s.lastMsg} numberOfLines={1}>
                      {lastMsg.sender_role === 'counselor' ? 'You: ' : ''}{lastMsg.content}
                    </Text>
                  )}
                </View>
                {unread > 0 && (
...
const s = StyleSheet.create({
  ...
  convCard: {
    ...
    borderRadius: 12, padding: 14, marginBottom: 10, borderWidth: 1, borderColor: '#E6EBF2',
  ...
  unreadBadge: {
    ...
    backgroundColor: '#E0785A', alignItems: 'center', justifyContent: 'center',
  ...
  inputField: {
    flex: 1, borderWidth: 1, borderColor: '#E6EBF2', borderRadius: 20,
    paddingHorizontal: 14, paddingVertical: 10, fontSize: 15,
    fontFamily: 'PublicSans_400Regular', color: '#17233D', maxHeight: 100, backgroundColor: '#F4F7FB',
  },
  sendBtn: {
    width: 48, height: 48, borderRadius: 24, backgroundColor: '#1E73CE',
