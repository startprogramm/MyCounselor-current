...
export default function ParentMessagesScreen() {
  ...
  async function sendMessage() {
    ...
    const { data, error } = await supabase
      .from('messages')
      .insert({ conversation_key: key, sender_id: user.id, sender_role: 'parent', content })
      .select()
      .single();
                ...
                <View style={{ flex: 1 }}>
                  ...
                  {lastMsg && (
                    <Text style={s.lastMsg} numberOfLines={1}>
                      {lastMsg.sender_role === 'parent' ? 'You: ' : ''}{lastMsg.content}
                    </Text>
                  )}
                </View>
                {unread > 0 && (
...
const s = StyleSheet.create({
  ...
  listHeaderSub: { fontSize: 12, color: '#6b7280', marginTop: 2 },
