...
export default function SignupParentScreen() {
  ...
  function validate() {
    ...
    if (!form.childrenNames.trim()) e.childrenNames = "Child's full name is required";
    setErrors(e);
  ...
  async function handleSubmit() {
    ...
    // Split comma-separated names, trim whitespace, filter empty strings
        ...
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          >
            <Text style={form.schoolId ? styles.pickerValue : styles.pickerPlaceholder}>
              {form.schoolId ? form.schoolName : "Select your child's school"}
            </Text>
          </TouchableOpacity>
          {errors.schoolId ? <Text style={styles.errorText}>{errors.schoolId}</Text> : null}

          <Text style={[styles.label, { marginTop: 14 }]}>Relationship</Text>
          <View style={styles.optionRow}>
            {RELATIONSHIPS.map(r => (
              <TouchableOpacity
                key={r}
                style={[styles.optionBtn, form.relationship === r && styles.optionBtnActive]}
                onPress={() => set('relationship', r)}
              >
                <Text
                  style={[styles.optionText, form.relationship === r && styles.optionTextActive]}
                >
                  {r}
                </Text>
              </TouchableOpacity>
          ...
          <Field
            ...
            onChangeText={v => set('childrenNames', v)}
            error={errors.childrenNames}
            placeholder="e.g. John Doe, Jane Doe"
            hint="Separate multiple children with commas. Name must match exactly what the student used when signing up."
          />
...
const styles = StyleSheet.create({
  ...
  optionBtnActive: { borderColor: '#1e40af', backgroundColor: '#eff6ff' },
