...
export type LetterAuthorRole = 'teacher' | 'counselor';
...
interface RecommendationLetterWorkspaceProps {
  authorRole: LetterAuthorRole;
  homeHref: string;
  homeLabel?: string;
}

export default function RecommendationLetterWorkspace(props: RecommendationLetterWorkspaceProps) {
  return (
    <Suspense fallback={null}>
      <RecommendationLetterWorkspaceInner {...props} />
    </Suspense>
  );
}

function RecommendationLetterWorkspaceInner({
  authorRole,
  homeHref,
  homeLabel = '← Back to Requests',
}: RecommendationLetterWorkspaceProps) {
  const { user } = useAuth();
  ...
  const chooseAngle = async (angle: Angle) => {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        ...(authorRole === 'teacher' ? { teacher_id: user.id } : { counselor_id: user.id }),
  ...
  const scheduleFieldSave = useCallback(
    (
      docId: number,
      field: 'opening' | 'body1' | 'body2' | 'closing' | 'merged_content',
      value: string
    ) => {
      ...
      saveTimers.current[key] = setTimeout(async () => {
        const updates: Partial<Record<typeof field, string>> = {};
        updates[field] = value;
  ...
  const generateSection = async (key: SectionKey) => {
    ...
    try {
      ...
      setGenError(
        err instanceof Error ? err.message : 'Unable to generate this section right now.'
      );
    } finally {
      setGenerating(null);
    }
  ...
  const mergeIntoDocument = async () => {
    if (!doc) return;
    const merged = SECTION_ORDER.map((key) => sections[key].trim())
      .filter(Boolean)
      .join('\n\n');
  ...
  const handleDownload = () => {
    ...
    downloadRecommendationLetterDoc({
      ...
      authorSubject: authorRole === 'teacher' ? user.subject : undefined,
  ...
  if (loadError || !requestRow) {
    return (
      <div className="max-w-3xl mx-auto space-y-4">
        ...
        <Link href={homeHref} className="text-sm text-primary hover:underline">
          {homeLabel}
        </Link>
      </div>
    );
      ...
      {!doc && (
        <Card className="p-6">
          ...
          <p className="text-sm text-muted-foreground mb-4">
            This shapes what the AI leans into when it helps you write — you can still cover
          ...
          <Card className="p-4 flex items-center justify-between gap-3 flex-wrap">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Write each part yourself, ask AI for help section by section, or generate a full
                draft to start from.
              </p>
            </div>
            <Button
