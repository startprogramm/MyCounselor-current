...
interface CounselorConversationPageProps {
  /** The role of the signed-in user, used as sender_role on outgoing messages. */
  role: 'teacher' | 'parent';
  subtitle: string;
}

export default function CounselorConversationPage({ role, subtitle }: CounselorConversationPageProps) {
  const { user } = useAuth();
