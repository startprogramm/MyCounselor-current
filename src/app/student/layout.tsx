...
import { gradeToAssessmentType } from '@/lib/career-assessment';
...
const studentNavItems: SidebarItem[] = [
  ...
  {
    label: 'Career Test',
    href: '/student/career-assessment',
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export default function StudentLayout({ children }: { children: React.ReactNode }) {
  ...
  const computeBadges = useCallback(async () => {
    ...
    const assessmentType = gradeToAssessmentType(user.gradeLevel);
    if (assessmentType) {
      const { data: assessmentRow } = await supabase
        ...
        .maybeSingle();
      if (!assessmentRow) counts['/student/career-assessment'] = 1;
    }
