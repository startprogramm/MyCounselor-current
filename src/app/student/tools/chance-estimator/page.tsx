  ...
  async function estimate() {
    ...
    try {
      const res = await fetch('/api/ai-chance-estimator', {
        ...
        body: JSON.stringify({ targetCollege: targetCollege.trim(), studentId: user?.id }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Estimation failed.');
      setResult(data.result);
      setHistory((prev) =>
        [data.result, ...prev.filter((h) => h.collegeName !== data.result.collegeName)].slice(0, 5)
      );
    } catch (err: unknown) {
        ...
        <div className="rounded-2xl border border-border bg-card p-5">
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
              Your Profile Used for Estimation
            </p>
            <Link href="/student/dashboard" className="text-xs text-primary hover:underline">
              Edit Profile
            </Link>
          </div>
            ...
            {studentProfile.intended_major && (
              <div className="rounded-lg bg-muted px-3 py-2 text-center">
                ...
                <p className="font-bold text-foreground text-sm">
                  {String(studentProfile.intended_major)}
                </p>
              </div>
            )}
            {!studentProfile.gpa_unweighted &&
              ...
              {history.map((h, i) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      setTargetCollege(h.collegeName);
                      setResult(h);
                    }}
          ...
          return (
            <div className="space-y-5 animate-[fadeIn_0.4s_ease]">
              ...
              <div className={`rounded-2xl border ${tc.border} bg-card overflow-hidden`}>
                <div
                  className="h-1.5"
                  style={{ background: `linear-gradient(to right, ${tc.color}, ${tc.color}88)` }}
                />
                <div className="p-6">
                  <div className="flex flex-col sm:flex-row items-center gap-6">
                    ...
                    <div className="flex-1 text-center sm:text-left">
                      ...
                      <div className="flex items-center gap-3 mt-2 justify-center sm:justify-start">
                        <span
                          className={`px-3 py-1 rounded-full text-sm font-bold ${tc.bg} ${tc.text}`}
                        >
                          {tc.label}
                        </span>
                  ...
                  <div className="space-y-3">
                    {result.actions.map((action, i) => {
                      ...
                      return (
                        ...
                        >
                          <span
                            className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase whitespace-nowrap flex-shrink-0 mt-0.5 ${ic.bg} ${ic.color}`}
                          >
                            {action.impact}
                          </span>
