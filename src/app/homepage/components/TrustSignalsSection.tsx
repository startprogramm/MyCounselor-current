  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto grid max-w-7xl gap-8 px-4 sm:px-6 lg:grid-cols-[1.05fr,1.45fr] lg:items-stretch lg:px-8">
        <div
          className={`rounded-2xl border border-border bg-card p-8 shadow-sm dark:border-slate-700 dark:bg-slate-800/60 animate-on-scroll-left ${isVisible ? 'animate-visible' : ''}`}
        >
          ...
          <div className="space-y-3">
            {[
              ...
              <div
                key={point}
                className="flex items-start gap-3 rounded-lg border border-border bg-background p-3 dark:border-slate-600 dark:bg-slate-800/70"
              >
                <Icon
          ...
          {trustSignals.map((signal, index) => (
            ...
            >
              <div className="mb-4 flex items-center justify-between">
                <div className="inline-flex h-12 w-12 items-center justify-center rounded-lg bg-primary/10 text-primary dark:bg-primary/20 dark:text-[#8AB4F8]">
                  <Icon name={signal.icon} size={24} variant="solid" />
                </div>
                <span className="rounded-full bg-muted px-3 py-1 text-xs font-semibold text-muted-foreground dark:bg-slate-700 dark:text-slate-200">
                  {signal.metric}
                </span>
              </div>
