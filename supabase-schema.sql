...
create table if not exists public.counselor_availability (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint counselor_availability_meeting_duration_check check (meeting_duration in (15, 30, 45, 60)),
  constraint counselor_availability_buffer_check check (buffer_time in (0, 5, 10, 15))
);
